...
" <<< "$raw" 2>/dev/null) || { log_warning "Could not parse review JSON"; return 0; }
    ...
    # ── Post as PR comment ──
    # Build a formatted markdown comment body using a temp file to avoid \n escaping
    local review_tmp; review_tmp=$(mktemp)
    {
        echo "## 🔍 AI Self-Review"
        echo ""
        printf '**Verdict:** '
        case "$verdict" in
            ...
            request_changes) echo "⚠️ Changes Requested" ;;
            *)               echo "💬 Comments" ;;
        esac
        echo ""
        ...
        if [ "$num_comments" -gt 0 ]; then
            echo "### Comments"
            echo ""
            echo "$review_json" | jq -r '.comments[] | "- **[\(.severity)]** `\(.file)` L\(.line): \(.body)"' 2>/dev/null || true
            echo ""
        fi
        echo "---"
        echo "*Self-review generated by Gemini AI (${model}).*"
    } > "$review_tmp"

    # Post as issue comment (simpler & works without fine-grained token scopes)
    local comment_body_json
    comment_body_json=$(python3 -c "import sys,json; print(json.dumps(sys.stdin.read()))" < "$review_tmp")
    rm -f "$review_tmp"
