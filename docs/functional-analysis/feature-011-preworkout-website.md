...
| Project |  |
|---|---|
| Primaire actor | Full stack webshop voor pre-workout supplementen |
| Secundaire actor | Klant |
...
Dit scherm toont de homepage met een hero sectie en een overzicht van beschikbare producten. Klanten kunnen producten filteren op basis van verschillende criteria.

### Productdetailpagina

![Productdetailpagina](feature-011-preworkout-website/page-3.png)

Dit scherm toont de detailpagina van een specifiek product. Hier kan de klant meer informatie vinden, het product toevoegen aan de winkelmand en eventueel favoriet maken.

### Checkout en order summary

![Checkout en order summary](feature-011-preworkout-website/page-4.png)

Dit scherm toont het checkout proces, inclusief velden voor verzendgegevens en betaalmethoden. De order summary toont een overzicht van de geselecteerde producten en de totale kosten.
...
Deze flow toont hoe klant, frontend, API, database en betalingsprovider samenwerken bij het afrekenen.
...
Onderstaande contracten geven een concrete basis voor backend implementatie. Alle endpoints onder order/admin vereisen een geldige token. Klantgebonden endpoints vereisen een geldige JWT access token.

| Endpoint | Request | Response |
|---|---|---|
| GET /api/products | Query: search, flavor, minPrice, maxPrice, caffeineMin, caffeineMax, isStock, sort, page, pageSize | 200: Paged list van actieve producten met id, name, price, image, stockStatus |
...
| POST /api/auth/login | email, password | 200: access token + user; 401: ongeldige token |
| GET /api/cart | JWT token | 200: cart items + totals |
| POST /api/cart/items | JWT token, quantity | 201: cart item; 400: quantity ongeldig; 409: onvoldoende voorraad |
| PATCH /api/cart/items/{id} | quantity, Path: cart item id | 200: aangepast item; 404: item niet gevonden |
